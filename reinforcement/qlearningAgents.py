...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        if maxQ == float("-inf"): maxQ = 0
        Qaprox = reward + (self.discount * maxQ)
        Qavg = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * Qaprox
        self.Qs[(state, action)] = Qavg
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for i in self.featExtractor.getFeatures(state, action):
            self.weights[i] = self.weights[i] + self.alpha * diff * feats[i]
